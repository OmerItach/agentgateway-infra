...
To integrate LLM providers like OpenAI, Gemini, Vertex AI, or Azure OpenAI:
1.  Create a `Secret` for the API key in `configs/llm-providers/`.
2.  Define an `AgentgatewayBackend` with the `ai` spec (e.g., `openai`, `gemini`, `vertexai`, or `azureopenai`).
